    ...
    <div className="grid grid-cols-3 gap-8 p-2">
      {TRACK_COLORS.map((item, index) => (
        <div
          ...
          className={`rounded-full cursor-pointer border-2 w-6 h-6 ${
            props.currentColor === item
              ? 'border-slate-200'
              : 'border-slate-600'
          }`}
          style={{ backgroundColor: item }} // using inline style to set background color
        />
      ))}
    </div>
  )
  ...
  return (
    ...
    >
      <span
        ...
        style={{ backgroundColor: currentColor }} // using inline style to set current color
      />
